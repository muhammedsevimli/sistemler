# Konsept Fabrikası · Otomatik Okuma Kuralı (AGENTS.md)
...
> Claude Code için aynı kural `CLAUDE.md` dosyasında, Cursor için `.cursor/rules/konsept-fabrikasi.mdc` dosyasında.
...
Çekirdek iş tek komutla ON-DEMAND çalışır: `CALISTIR.md` Adım 1 komutunu ver, 50 konsept `ciktilar/`'a düşer. "Her sabah masaüstüne bıraksın" istiyorsan bu tek komutu her sabah otomatik çalıştıran bir görev kurarsın (Windows Görev Zamanlayıcı ya da Mac cron). Kurulum: `otomasyon/KUR-otomasyon.md`. Sihirli görünmez bir otomasyon yok: her sabah dosyası, aynı tek komutun zamanlanmış hâlidir.
